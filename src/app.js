...
app.get('/api/get-env', (req, res) => {
    ...
    if(!secretCode){
      return res.status(500).json({ error: 'Secret code not found' })
    }
    const encryptedSecretCode=encryptString(secretCode)
    return res.status(200).json({ secret: encryptedSecretCode })
});
